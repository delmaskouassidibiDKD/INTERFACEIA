...
export default function Strands({
  ...
  useEffect(() => {
    ...
    try {
      const width = Math.max(ctn.offsetWidth, 100);
      const height = Math.max(ctn.offsetHeight, 100);
      ...
      if ((geometry as any).attributes && (geometry as any).attributes.uv) {
      ...
      function resize() {
        if (!ctn) return;
        const w = Math.max(ctn.offsetWidth, 100);
        const h = Math.max(ctn.offsetHeight, 100);
        try {
      ...
      cleanupFunc = () => {
        ...
        try {
          ...
          // Ignore context lose errors
        }
      };
    } catch (e) {
      console.warn('Strands WebGL initialization skipped:', e);
    }
